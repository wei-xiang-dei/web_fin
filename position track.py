...
data_x=np.zeros((3,300))
data_y=np.zeros((3,300))
data_z=np.zeros((3,300))
data_all=np.zeros((300,9))
in_li=None
tr1=IMUTracker(sampling=34)
p=None
ini_cnt=0
def p_update():
    global in_li ,p,p_x,p_y,p_z
    global tr1
    global ini_cnt
    line=serie.readline()
    if(ini_cnt==0):
    ...
    acc = np.array(tab[0:3] )#read and store the 3 values of acc according to how you send your data from arduino
    gyr = np.array(tab[3:6]) #read the 3 values of gyr
    mag = np.array(tab[6:9]) #read the 3 values of mag  
    data_x[:,:-1]=data_x[:,1:]
    data_y[:,:-1]=data_y[:,1:]
    data_z[:,:-1]=data_z[:,1:]
    data_x[:,-1]=acc
    ...
    if(ini_cnt%30==0):
        print(data_all,'\n')
    data_all[-1]=np.concatenate([gyr,acc,mag])
    ini_cnt=ini_cnt+1
    if(ini_cnt>300):
        a_nav, orix, oriy, oriz = tr1.attitudeTrack(data_all[200:], in_li)
        a_nav_filtered = tr1.removeAccErr(a_nav, filter=False)
        v = tr1.zupt(a_nav_filtered, threshold=0.2)
        p=tr1.positionTrack(a_nav_filtered,v)
        p_x=p[:,0]
        p_y=p[:,1]
        p_z=p[:,2]
        ...
        in_li = tr1.initialize(data_all[50:200])
        # need another 300 sample
        a_nav, orix, oriy, oriz = tr1.attitudeTrack(data_all[200:], in_li)
        a_nav_filtered = tr1.removeAccErr(a_nav, filter=False)
        v = tr1.zupt(a_nav_filtered, threshold=0.2)
        p=tr1.positionTrack(a_nav_filtered,v)
...
flag=0
@tl.job(interval=timedelta(microseconds=34))
def the_job():
    p_update()
...
if __name__ == '__main__':
    ...
    while True:
        try:
            if(ini_cnt>300 and flag==0):
                
                a1=FuncAnimation(plt.gcf(),plotdate,interval=500)
                plt.show()
                flag=1
            time.sleep(0.034)
